...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # switch between two models f.e. train one with only 1 and one with only 0.5
...
@torch.no_grad()
def evaluate(data_loader, model, device, use_amp=False, p_list=[1]*36):
    #TODO inlucde get_accelerator from hydravit validate_macs (mit hardcoded p_list)
    criterion = torch.nn.CrossEntropyLoss()

    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'

    # if isinstance(p_list[0], list): #FIXME worng like this
    #     p_list = p_list[epoch%len(p_list)]
